...
function lintParagraph(paragraph){
  ...
  Print('Total Words in Story: '+storyWords.length);
  ...
  Print('Necessary Word Count: '+necessaryWords);
  ...
  Print('Over Used Word Count: '+(necessaryWords-betterWords.length));

  Print('Total Sentences Used: '+ storyWords.reduce((acc,word)=>{
...
const findMostUsedWords=(paragraphWords)=>{
  ...
  Print('  The Most Used Word: '+mostUsedWord);
  Print('          Times Used: '+current);
}
Print('\nFormatted Paragraph:\n'+lintParagraph(story));
