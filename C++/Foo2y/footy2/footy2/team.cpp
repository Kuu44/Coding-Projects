...
public:
    ...
    void move2(float x,float y,team* an_team,ball* football){
        if(noCollisionAfter(x,y,an_team)){
            players[aktv].setPosition();
        }
    }
    void move(Keyboard::Key left,Keyboard::Key right,Keyboard::Key up,Keyboard::Key down,Keyboard::Key changeKey,team* an_team,ball* football)
    {
        Vector2f input(0,0);
        float speed=0.07f;
        ...
        move2(input.x,input.y,an_team,football);
        //return players;
    }
    void update(){
        for(int i=0;i<TeamSize;i++){
